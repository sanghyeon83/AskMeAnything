...
> 작성일: 2026-07-27 / 최종 갱신: 2026-08-10

> **2026-08-10 정리**: 재부팅 후 **8개 세션이 전부 빈 대화로 떴다.** 원인은 `-c` 이어받기 전원 실패 → 빈 대화 폴백(4장 참고). 이후 복구해 다시 띄웠고, 이때 **세션이 두 세대로 겹쳐 보이는 현상**을 겪었다. 진단·구분법을 4장 끝에 정리했다.
...
### 2026-08-10 추가 (재부팅 후 8개 전부 빈 대화 — 폴백 전원 발동)

재부팅했더니 **태블릿의 8개 세션이 전부 빈 대화**였다. 대화가 삭제된 게 아니라, `-c`가 전원 실패해 60행의 **빈 대화 폴백이 8개 모두에서 동시에 걸린 것**이다.

**폴백인지 판별하는 법 — 세션 ID가 새로 발급됐는지 본다.**

`-c`가 성공하면 **기존 클라우드 세션 ID를 그대로 물고 `updated_at`만 갱신**된다. 실제로 `Tokbell HA_로컬`은 07-27 생성분이 08-07까지 같은 ID로 계속 갱신되고 있었다. 반면 08-10에는 **전부 새 ID가 발급**됐고 사용량이 0이었다 → 이어받기가 아니라 새 대화라는 확증.
...
**⚠️ 방치하면 복구 불가가 된다.** 76행 그대로다. 빈 폴백 세션이 "가장 최근 대화"가 되므로, 다음 재부팅부터 `-c`는 원본이 아니라 그 빈 세션을 이어받는다. 원본과 점점 멀어지므로 **발견 즉시 손대야 한다.**

**원인은 미확정.** 8개가 한꺼번에 죽은 걸 설명하려면 개별 원인이 아니라 공통 원인이어야 한다.

1. **부팅 20초 후 실행 시점에 네트워크 미완성** — `-c`는 클라우드 세션을 서버에서 조회해야 한다(157행에서 확인된 동작). 조회 실패 → 8개 동시 폴백. 가장 유력. 대응은 `.vbs`의 대기 시간을 60~90초로 늘리는 것
2. **`-c`의 "최근 대화" 판정 범위 밖** — 마지막 활동 08-07, 발생일 08-10으로 3일 차. 08-06 Tokbell 사례(9일)보다 짧아 상대적으로 약한 후보

구분은 125~132행의 진단 방법으로 `rc.err.log`를 보면 된다. **다음에 또 겪으면 폴백을 고치기 전에 이 로그부터 남길 것.** 이번에는 복구를 먼저 해버려서 원인 확정에 실패했다.

### ⚠️ 세션이 두 세대로 겹쳐 보이는 현상 (2026-08-10)

복구 작업으로 세션을 다시 띄우면, **죽은 1세대와 새 2세대가 앱 목록에 동시에 남는다.** 이름이 같아서 어느 쪽이 진짜인지 목록만 봐서는 모른다.

| | 1세대 (`.vbs` 자동 실행) | 2세대 (복구 시 재기동) |
|---|---|---|
| 태그 | `remote-control-cli` | `remote-control-repl` |
| origin | `claude_code_cli` | `claude_code_sdk` |
| `external_metadata` | 폴더/브랜치 정보 있음 | **비어 있음** |
| 앱 표시 위치 | 정렬해 둔 프로젝트 그룹 | **기타** |

**함정: 껍데기가 제자리에 앉고 알맹이가 "기타"로 밀린다.** 2세대는 작업 폴더·브랜치를 아직 서버에 안 올린 상태라 앱이 프로젝트를 몰라 "기타"로 분류한다. 반대로 1세대는 그 정보를 갖고 있어 원래 자리를 차지한다. **정렬된 자리에 있다고 진짜가 아니다 — 대화 내용으로 판단할 것.**

→ 대화가 살아있는 쪽을 확인했으면 **1세대 빈 세션은 아카이브해서 목록에서 치운다.** 08-10에는 6개(Tokbell HA / Tokbell / Tokbell Sender / Rabbit Typing / Webs Madang / Morning Economy)를 아카이브했다. 아카이브는 되돌릴 수 있다.

> ⚠️ 복구 중에는 **PC의 remote-control 세션과 클라우드 세션이 같은 `.vbs`를 동시에 건드리지 않도록** 할 것. 08-10에 양쪽이 동시에 이 문제를 보고 있었다. `.vbs` 수정은 PC 쪽에서만 한다.
...
- **재부팅 직후에는 세션이 "떴는지"가 아니라 "대화를 물고 왔는지"를 확인할 것.** 목록에 이름이 보여도 빈 폴백일 수 있다 (4장 08-10 항목). 가장 빠른 확인법은 아무 세션에 "우리가 지금까지 뭐 하고 있었지?"라고 물어보는 것
