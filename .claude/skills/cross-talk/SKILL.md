...
2. **2차 — 트리거 중계 (1차 목록에 상대가 안 보일 때).** claude-code-remote MCP 도구를 쓴다. 도구가 안 보이면 ToolSearch로 "list_sessions", "create_trigger", "fire_trigger", "delete_trigger", "get_session"을 로드한다.
   ...
   > ⚠️ **본문이 길면 JSON이 잘려 파싱 오류가 난다.** 한글은 `\uXXXX`로 이스케이프되며 약 6배로 불어난다. **한글 300~400자 이내**로 끊어 여러 통으로 보낸다 (실패 사례가 5,194바이트였다. 이전 판의 "2000자"는 escape 후 크기를 과소평가한 수치였다).
...
- **데스크톱 앱 로컬 세션도 수신은 된다** (2026-09-07 실측 정정 — 이전 판의 "상대가 될 수 없다"는 과한 서술이었다). 브리지 세션 ID로 보내면 정상 수신된다. 닿지 않으면 사용자 대화창에 남겨 사용자가 전달하게 한다.
- ⚠️ **무엇이 바뀌고 무엇이 안 바뀌는지** (2026-09-07 재부팅 실측으로 정정):

  | 대상 | 바뀌나 | 근거 |
  |---|---|---|
  | 브리지 `session_` ID | **대화가 이어지는 한 유지. 재부팅도 건넌다** | 윈도우 재부팅 전후 `session_01ERZAmF…` 동일. 맥이 같은 ID를 62분 간격으로 두 번 써서 둘 다 도달 |
  | `ListAgents` 피어 이름 | **매번 바뀐다** | 재부팅 전후 `askmeanything-e4` → `askmeanything-b0` |

  회전하는 쪽은 **이름**이다. 앞서 "브리지 ID가 수십초~수분 단위로 회전한다"고 적었던 것은 틀렸다 —
  그때 본 `session_01Mbr8ka…` → `session_01ERZAmF…` 변화는 세션이 **새로 떴기 때문**이지 시간이 지나서가 아니다.
  그러므로 **이름으로 상대를 특정하려 들면 실패한다.** 이름은 조회 시점에만 유효하고 안정적인 것은 ID 쪽이다.
  다만 세션이 새로 뜨면 ID도 새것이 되므로 하드코딩은 여전히 금지하고, 발사 직전에 조회한다.
